...
def main():
    ...
    if args.m <= 3:
        ...
        target_script = 'adaptive_annealing.py'
        print(f"--- m = {args.m} > 3, calling {target_script} with adaptive surrogate model ---")
        base_command.append(target_script)

        # Add all relevant arguments for adaptive annealing
        base_command.extend(['-k', str(args.k)])
        base_command.extend(['-n', str(args.n)])
        base_command.extend(['-m', str(args.m)])

        # Add optional arguments if provided, letting annealing script handle defaults
        if args.element_min is not None:
            base_command.extend(['--element-min', str(args.element_min)])
        if args.element_max is not None:
            base_command.extend(['--element-max', str(args.element_max)])
        ...
        # Add adaptive model specific arguments
        if args.strategy:
            base_command.extend(['--strategy', args.strategy])
        if args.sample_rate:
            base_command.extend(['--sample-rate', str(args.sample_rate)])
        if args.verify_final:
            base_command.append('--verify-final')
